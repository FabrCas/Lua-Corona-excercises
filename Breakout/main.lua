...
local currentLevel
local vx= 3                              -- default velocity for the ball
local vy= -3
local gameevent = ""
local isSimulator = "simulator" ==          --to simulate on corona
...
function loadGame (event)
  if event.target.name == "playbutton" then
    ...
    playBtn: removeEventListener ("tap", loadGame)       --remove playbutton from the scene
  end
end
...
function dragPaddle (event)
  --for the simulator that isn'enabled to use the accelerometer events
  if isSimulator then 
    ...
    elseif ((paddle.x + paddle.width * 0.5 ) >
...
background:removeEventListener("tap", startGame)                       --remove "tap" event for startGame() 
gameListeners("add")
end
...
function gameLevel1 () 
  currentLevel = 1 
  bricks:toFront() --it appears in front of the bg
...
if event.other.name == "brick" and ball.x + ball.width *0.5 < event.other.x + event.other.width *0.5 then
  vx= -vx
elseif event.other.name == "brick" and ball.x + ball.width * 0.5 >= event.other.x + event.other.width *0 then
  vx=vx
end
if event.other.name == "brick" then vy = vy* -1
